...
class ThreadMqttHandler(threading.Thread):
    """Manages MQTT connection for a single Pool Controller coordinator device."""
    ...
    def run(self):
        try:
            self.mqtt_client = mqtt.Client(
                client_id=f"indigo-pentair-pool-{self.dev_id}",
                clean_session=True,
    ...
    def publish(self, topic, payload):
        if self.mqtt_client and self.connected:
            full_topic = f"{self.root_topic}/{topic}"
            if isinstance(payload, dict):
                payload = json.dumps(payload)
            self.mqtt_client.publish(full_topic, payload, qos=1)
